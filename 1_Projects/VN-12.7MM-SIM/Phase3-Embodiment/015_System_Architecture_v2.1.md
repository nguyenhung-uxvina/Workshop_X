...
**Document**: VN-12.7MM-SIM-015-ARCH | **Version**: 2.1 | **Date**: 2026-03-20
...
| Subsystem count | 7 (informal) | 9 (formal, with spatial allocation; SS10 Recoil added v2.1) |
...
│     │   ┌──────────────┐  ┌─────────────┐  ┌──────────────┐  ┌──────────────┐      │   │
│     │   │• 3x 27" QHD  │  │• 5.1 system │  │• Status LEDs │  │• Monitor     │      │   │
│     │   │• Monitor arms│  │• 300W amp   │  │• HUD overlay │  │• Keyboard    │      │   │
│     │   │• Cable mgmt  │  │• Subwoofer  │  │              │  │• Control SW  │      │   │
│     │   └──────────────┘  └─────────────┘  └──────────────┘  └──────────────┘      │   │
│     │                                                                               │   │
│     │   SS10: RECOIL (v2.1 — Channel B, RC-A Pneumatic per Doc 022)                │   │
│     │   ┌──────────────────────────────────────────────────────────────────────┐    │   │
│     │   │• Pneumatic cyl 54mm bore, 25mm stroke (traverse platform mount)     │    │   │
│     │   │• Fast solenoid valve ≤15ms (Parker D311 / Festo MHJ)               │    │   │
│     │   │• Pressure regulator 8-10 bar + pressure transducer (data capture)   │    │   │
│     │   │• Linkage to bolt carrier replica + exhaust muffler                  │    │   │
│     │   └──────────────────────────────────────────────────────────────────────┘    │   │
│     │                                                                               │   │
│     └───────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                         │
...
│     │                          → 24VDC PSU (150W) → brakes + solenoid valve         │   │
│     │ 220VAC → Compressor (1.5kW, separate circuit, remote location)                │   │
...
| Parameter | Specification | Requirement |
|-----------|--------------|-------------|
| **Function** | Provide visual status cues (haptic recoil moved to SS10) | F5.5, F5.2, F6.8 |
...
| **Local content** | 100% -- LEDs locally sourceable | -- |

**Note (v2.1):** ERM vibration motor removed. Per Doc 022 analysis, ERM on receiver = wrong location for mounted weapon (energy dissipates before reaching gunner through mount structure). Haptic recoil now provided by SS10 Pneumatic Recoil acting on entire traverse platform — physically correct for mounted 12.7mm.

## SS10: Recoil Subsystem (v2.1 — Channel B)

*Added per Doc 022 v1.1 CEO selection: RC-A Pneumatic Standard.*
*Two-Channel Architecture: SS3 = Channel A (mount resistance), SS10 = Channel B (firing vibration).*

| Parameter | Specification | Requirement |
|-----------|--------------|-------------|
| **Function** | Generate per-shot recoil impulse at weapon cyclic rate | F-005, TE-001, K-010 |
| **Architecture** | RC-A Pneumatic Standard (Doc 022, VDI 2225: 78.6%) | -- |
| **Pneumatic cylinder** | Double-acting, 54mm bore, 25mm stroke, 10 bar rated | Force: ≥1,400 N peak (70% of 2,000 N reference) |
| **Solenoid valve** | 3/2 fast-acting, ≤15 ms full open, 24VDC coil | Cycling: 450-600 RPM (7.5-10 Hz) |
| **Solenoid model** | Parker D311 or Festo MHJ series | -- |
| **Pressure regulator** | 0-10 bar adjustable, wall-mount, with gauge | Force tunability for different weapon profiles |
| **Pressure transducer** | 0-16 bar, 4-20mA or 0-5V, 1 kHz response | Data capture for I-08 ACH (MOUNT "T") |
| **Muffler** | Sintered bronze, on exhaust port | E-007 target: mechanism ≤70 dB(A) at 1m |
| **Linkage** | Rod + clevis to bolt carrier replica, adjustable stroke limit | Integration with SS1 gun replica |
| **Air supply** | Shop compressor 2 HP, 50L receiver, 8-10 bar, remote location | OP-002 continuous duty |
| **Air line routing** | 8mm OD PU tubing, through pedestal + rotary union on traverse | 360° traverse capability |
| **Synchronization** | I/O board GPIO → solenoid driver, ≤10 ms trigger-to-impulse | S-004 latency budget |
| **Fallback** | Solenoid de-energize on E-stop → cylinder depressurizes → safe | SF-001 |
| **Mass** | Cylinder 1.5 kg, valve 0.5 kg, plumbing 1.0 kg, transducer 0.2 kg = **3.2 kg** | -- |
| **Power** | Solenoid: 24V × 1A = 24W peak (intermittent). Compressor: 1.5 kW (separate circuit) | E-002 (trainer only, excl. compressor) |
| **Local content** | Cylinder: local. Valve: import. Regulator: local. Compressor: local. Plumbing: local. ~60% | -- |
| **Physical gate** | T2: Cylinder force test (AS-001), T3: Valve speed test (AS-002) | Aligned with brake test 2026-04-05 |
| **Electric swap path** | IF-027 mechanical mount + IF-028 electrical connector designed for linear motor replacement in I-06/I-08 | MOUNT "U" principle |
...
| SS7 Feedback | S-012 (HUD), MT-006 (partial) | 2 |
| SS8 Instructor | OP-003..012, S-013..S-019, OP-012 | 14 |
| SS10 Recoil | F-005, TE-001, TE-002, K-010, FC-004, FC-005 | 6 |
...
| SS7 Feedback | LEDs + wiring (0.2) | 0.2 |
| SS8 Instructor | Monitor (5), keyboard+mouse (1) | 6 |
| SS10 Recoil | Cylinder (1.5), valve (0.5), plumbing (1.0), transducer (0.2) | 3.2 |
| Cabling & misc | Power cables, signal cables, connectors, cable management | 3 |
| **TOTAL** | | **~229 kg** |
| **Requirement** | TR-002: max shipping weight <=500 kg | **PASS** (margin: 271 kg) |
| **Structural** | F-004: support 150 kg operator + 80 kg weapon replica = 230 kg | Base designed for 350 kg total (229 system + 150 operator = 379 kg -- OK with safety factor) |
...
| 10 | Mount pneumatic cylinder + linkage on traverse platform | SS10 | M6 wrench, PTFE tape | 0.5 hr |
| 10a | Install solenoid valve + regulator + muffler | SS10 | Spanners, tubing cutter | 0.3 hr |
| 10b | Route air line through pedestal + rotary union | SS10 | Push-fit connectors | 0.5 hr |
| 10c | Install pressure transducer, wire to I/O board | SS10 | Wiring | 0.2 hr |
...
| **TOTAL** | | | **Standard tools only (AS-002)** | **~9.5 hrs (2 techs, AS-001)** |

Module count: **6 major modules** (SS1 mech, SS2+SS3 sensor/brake, SS4 computing, SS5 visual, SS6 audio, SS10 recoil) -- meets AS-003.
...
| SS7 Feedback | Status LEDs, HUD overlay | 9 | -- |
| SS8 Instructor | PC + software | 8 | -- |
| SS10 Recoil | Pneumatic cylinder + fast solenoid | 7-8 | **Prototype test (Doc 022 T2/T3) validates force + cycling rate** |

**All components TRL >=7.** No TRL <6 items. SS3 brake and SS10 recoil cylinder are components requiring physical validation (Doc 014, Doc 022).
...
| 2.1 | 2026-03-20 | KN + AI | Added SS10 Recoil (RC-A Pneumatic per Doc 022 v1.1). Removed ERM from SS7. Updated block diagram, mass budget (+3 kg), TRL table, assembly sequence (+1.5 hr), traceability (+6 reqs to SS10). Two-Channel architecture: SS3 = Channel A, SS10 = Channel B. |

---

**NEXT**: Document 016 -- Interface Control Document (ICD) v1.1

---

*VN-12.7MM-SIM-015 System Architecture v2.1*
