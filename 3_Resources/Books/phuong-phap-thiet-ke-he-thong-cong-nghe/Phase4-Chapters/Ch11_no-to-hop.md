...
Ba thứ sẽ được giao. Một: con số thật của vụ nổ, có nguyên văn trong nguồn — và ghi rõ những con số mà
nguồn **không** nêu. Hai: một bảng bốn cột đối chiếu bốn cách né của bốn thế hệ, mỗi cách kèm cái nó hy
sinh. Ba: **nghịch lý thông tin** — chốt muộn thì nổ tổ hợp, chốt sớm thì chốt khi chưa biết gì — và bằng
chứng rằng không phương pháp nào trong corpus thoát được nó; tất cả chỉ chọn chỗ trả giá.
...
Đọc kỹ cấu trúc câu. Vế đầu là lời khen: phương pháp này **triệt tiêu rủi ro bỏ sót giải pháp mới**. Vế
sau rút lại lời khen ấy: vì không quét hết được nên **phải giới hạn số chức năng con**. Hai vế trong một
câu, nối bằng chữ *therefore*. Công cụ được ca ngợi vì không bỏ sót gì, rồi được khuyên dùng theo cách bảo
...
Một ghi chú về kỷ luật trích dẫn. Tệp khám phá của phả hệ VDI 2221 mô tả một ca ứng dụng thật, trong đó
nhóm thiết kế dựng ma trận hình thái với các lựa chọn A/B/C cho từng hàng rồi chỉ vẽ ra một số ít biến thể
— minh hoạ đúng luận điểm chương này. Nhưng nó **không kèm câu nguyên văn nào** cho các phép đếm đó, nên
chương này không dùng con số nào từ ca ấy. Cái đáng nói không phải con số bị bỏ, mà là mẫu hình: báo cáo
ứng dụng ghi *cái đã chọn* và hiếm khi ghi *cái đã bị cắt cùng lý do* — chính dữ liệu để đo mức nghiêm
trọng của vụ nổ lại là thứ ít được ghi nhất.
...
gom nhóm các thông số theo thứ tự ưu tiên giảm dần rồi đánh giá từng nhóm. Quy trình tám bước của
Roozenburg & Eekels đặt việc rút gọn thành bước riêng — bước 5 dùng chiến lược đánh giá để giới hạn số giải
pháp nguyên lý — rồi kết ở bước 7 bằng một trần dưới: `"choose a limited number of principal solutions
...
Mỗi ô nhận thêm một điểm rủi ro, chấm theo đúng ba câu hỏi — khoảng trống tri thức trong R&D, vấn đề công
nghệ hoặc sản xuất đòi vốn mới, vấn đề bảo trì gây bất mãn — và quy tắc quy đổi là cơ học:
...
chấm. Khác biệt duy nhất là nhát cắt được đặt ở chỗ **có ghi chép** hay ở chỗ **không ai nhìn thấy** — một
khác biệt về tổ chức, không phải về kỹ thuật. (Việc xếp bốn thế hệ theo trục "vị trí nhát cắt" là khung
phân loại của cuốn sách; không nguồn nào trong corpus đặt bốn phương pháp cạnh nhau theo trục này.)

> **Đào sâu: các hàng của ma trận hình thái không độc lập**
>
> Phép nhân ở đầu chương dựa trên một giả định: mỗi hàng chọn được tự do. Nếu đúng, số tổ hợp là tích các
> hàng. Nếu sai, con số vừa quá lớn — nhiều tổ hợp bất khả thi — vừa quá nhỏ, vì một bộ phận có thể gánh
> nhiều chức năng, thứ bảng không biểu diễn nổi.
>
> Nghiên cứu thực nghiệm trong nguồn bác giả định ấy trực tiếp:
> `"Börekçi (2018) investigated its effectiveness in terms of design divergence. ... This research was done
> with 50 grad students. They have found out that the participants tend to create solutions to more than
> one sub-function at once and continue exploring solutions for sub-functions they have already moved away
> from; this means that for the participant, the components of a product (in this case a coffee maker) were
> not independent, rather interdependent."` [29]
>
> Quy mô của tuyến nghiên cứu ấy cũng có nguyên văn:
...
>
> Và giới hạn biểu diễn thì chính nguồn gọi tên:
> `"Traditional design methods generally do not encourage multifunctioning design. For example,
> morphological charts imply that all the individual subfunctions should have their own individual function
> carrier because separate boxes are allocated for each subfunction."` [29]
>
> Hệ quả: vụ nổ tổ hợp **không** phải một bài toán đếm sạch sẽ mà thuật toán tốt hơn sẽ dọn được. Không
> gian thật vừa lớn hơn khả năng duyệt, vừa méo so với mô hình dùng để đếm nó. Đó là lý do sâu xa khiến
> DSO sinh ra tổ hợp bất khả thi trong *mọi* dự án thử nghiệm: nó tối ưu trên một mô hình mà hình dạng bảng
> đã giả định sai từ đầu. Cách đọc này là suy luận của tác giả; nguồn ghi hai sự kiện ở hai chỗ, không nối.
...
Đọc chậm câu thứ hai. Biến độc lập không phải lượng thông tin, không phải chất lượng công cụ, không phải
...
Chính ICDM ghi lại điều này về hệ đo lường chất lượng thiết kế của mình:

> `"We can compare design process data only against forecasts or expectations. These metrics are subjective
> and subjected to personal influence, power and pressure."` [46]

*Personal influence, power and pressure.* Một phương pháp bán định lượng, tự trang bị công thức tổng-tích
và hàm hài lòng phi tuyến, thừa nhận rằng ở pha khái niệm các chỉ số của nó chịu tác động của quyền lực —
và đây là tác giả tự viết, không phải lời phê từ bên ngoài. Nó khép lại lập luận của chương: vụ nổ tổ hợp
là số học, nhưng nhát cắt dùng để né nó là một **hành vi tổ chức**, do một người có vị trí thực hiện, dưới
áp lực, không để lại hồ sơ.

Bằng chứng về việc các thang chấm được dùng thế nào ngoài đời khép nốt vòng:
...
Một chi tiết nữa của Chương 10 nay đọc ra khác. Ma trận Pugh có một chế độ hỏng đã ghi trong nguồn: khi một
phương án vượt lên hạng nhất, nó phải thay chỗ Datum và toàn bộ vòng so sánh chạy lại —
`"...then it's going to take the place of the DATM is that one is going to be the DATM and you compare the
rest with that one"` [34]. Ở Chương 10 đó là một mẹo tinh chỉnh; ở đây nó là vế thứ nhất của nghịch lý thu
nhỏ: mỗi vòng lặp làm ta biết thêm, và biết thêm thì phải chấm lại từ đầu. Cái ngăn vòng lặp ấy chạy vô hạn
không phải tiêu chí hội tụ nằm trong phương pháp — mà là một người trong phòng nói "đủ rồi". Cách đọc này
là của tác giả; nguồn mô tả thao tác, không rút ra hệ quả đó.

---

## Chỗ cuốn sách đổi giọng

Phần III mô tả công cụ: sinh giải pháp, chấm điểm, và bây giờ là chỗ hai việc ấy không khớp nhau. Chương
này là bản lề vì nó là lần đầu tiên câu trả lời không còn là *công cụ này làm gì*, mà là *không công cụ nào
làm được, và cách né lộ ra giả định gì*.

Từ **Chương 12**, giọng đổi: cuốn sách thôi mô tả và bắt đầu chất vấn. Câu hỏi mở đầu Phần IV đến thẳng từ
chỗ chương này dừng — nếu mọi phương pháp đều phải cắt bằng tay ở thời điểm tri thức thấp nhất, thì mô tả
tuyến tính "từ trừu tượng xuống cụ thể" mà cả bốn thế hệ dùng làm khung có thật sự mô tả đúng cái người ta
làm không? Chương 12 mang bằng chứng thực nghiệm của tuyến phê bình — FBS của Kannengiesser & Gero, Motte,
Jensen & Andreasen, *Design Methods in Practice* — và phân biệt một điều quyết định phần còn lại của sách:
**"phương pháp không mô tả đúng" không đồng nghĩa với "phương pháp vô dụng"**.

Đây là một thay đổi trong hợp đồng đọc. Từ đây, mỗi chương ít trả lời "làm thế nào" hơn và nhiều "cái này
đặt cược vào điều gì" hơn. Ai đọc để lấy quy trình chạy được nên đọc kỹ mục *Áp dụng ở Xưởng* của các
chương sau và lướt phần lập luận. Ai đọc để hiểu vì sao lần trước áp quy trình mới mà không ăn — Phần IV
là phần viết cho người đó.
...
0 nghĩa là một ô 0 giết cả cột — hãy chắc rằng điểm 0 được gán vì đã hỏi đủ ba câu hỏi rủi ro, không phải
vì người chấm chưa từng làm nguyên lý đó bao giờ.
...
### 4. Cấu phần không lập lịch được thì không nằm trong lịch

> `"...at level 4, an unknown number of development cycles is required ... therefore no project can include
> a level-4 component. Such components are covered under a separate research effort before the project
> starts."` [55]

**Vấn đề nó giải:** một cấu phần chưa có chứng minh khả thi kéo theo số vòng lặp không dự báo được, và nó
làm hỏng lịch của cả những phần vốn lành mạnh. **Cách áp:** trước khi khoá lịch, rà từng cấu phần bằng một
câu hỏi — *đã có ai, ở đâu, làm cho cái này chạy chưa?* Cái nào trả lời "chưa" thì tách thành nhánh nghiên
cứu riêng có ngân sách riêng, còn lịch sản phẩm chạy trên phương án dự phòng. **Bẫy:** áp luật này quá rộng
thì xưởng không bao giờ làm cái mới nữa. Nó là luật về **lịch**, không phải luật về **danh mục** — cấu phần
cấp bốn vẫn được nghiên cứu, chỉ là không được nằm trên đường găng.

### 5. Trần cho bảng yêu cầu, và một danh sách chờ thay vì cái sọt rác

> `"A matrix of more than 20x20 or 15x25 is impractical to handle because it consumes too much time."` [46]

**Vấn đề nó giải:** bảng yêu cầu phình ra tới khi không ai đọc hết, và khi không ai đọc hết thì độ đầy đủ
của nó chỉ còn là hình thức. **Cách áp:** đặt trần cho số hàng nhu cầu và số cột đặc tính ngay lúc lập
bảng; mọi thứ vượt trần đi vào một **danh sách chờ có tên**, không bị xoá. **Bẫy:** danh sách chờ thành
nghĩa địa nếu không có mốc mở lại — buộc nó vào một sự kiện có thật, buổi rà trước khi chốt bố cục tổng
thể, chứ đừng buộc vào một ngày trên lịch.
...
- **Chỗ là suy luận của tác giả, không phải của nguồn:**
  ...
  - Xếp hai chiến lược rút gọn của tuyến Delft và quy trình tám bước Roozenburg & Eekels vào cùng "cột
