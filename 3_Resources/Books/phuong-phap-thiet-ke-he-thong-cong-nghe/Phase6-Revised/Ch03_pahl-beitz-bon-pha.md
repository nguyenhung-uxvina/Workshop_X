...
- **Nguồn đã dùng:** `[1]`, `[31]`, `[33]`, `[37]`, `[43]`, `[45]`, `[47]`, `[50]`.
  ...
  định về hiệu quả đều đã bắc ra khỏi `[1]` — nhưng không phải chỗ nào cũng bắc được sang nhiều tuyến
  độc lập:** mệnh đề chi phí khoá sớm dùng `[45]`, `[47]`, `[50]`, và theo `_danh_muc_nguon.md` thì cả
  ba nằm trong **một nhóm duy nhất, tuyến ICDM của Hari & Weiss**, nên đó là *một* tuyến đối chứng chứ
  không phải ba; khoảng cách mô hình–hành vi thì bắc được sang ba tuyến khác nhau, `[31]`, `[43]`, `[33]`. Và chương ghi rõ rằng khẳng định *phương pháp
...
- **Con số có nguyên văn:**
  ...
  - *thang 0–10 và 0–4* — `"Cost–Benefit Analysis employs a range from 0 to 10; Guideline VDI 2225 a
    range from 0 to 4"` `[1]`
  ...
  - *`fifteen` = 0, `fifteen steps` = 0, `15 steps` = 0 trên 1.167.487 ký tự* — kết quả đếm chuỗi trên
    toàn bộ văn bản trích ra được của `[1]`. 1.167.487 là số ký tự mà `_danh_muc_nguon.md` ghi cho tài
    liệu này; cuốn sách dùng đúng một phép đo đó ở mọi chỗ. Ghi ở `_cau_hoi_mo_DA_GIAI.md` mục Q1
...
- **Khối bằng chứng nhường cho chương khác (khử trùng lặp P6):** số liệu thực nghiệm sFBS của `[31]`
  (số bước ánh xạ, số phiên, thời lượng phiên) và cặp câu về 60% thời gian pha ý tưởng **có nhà chính ở
  Chương 02**; chương này giữ kết luận và ràng buộc đối tượng đo, kèm câu trỏ sang. Không bằng chứng nào
  bị mất, chỉ bị in một lần thay vì hai.
